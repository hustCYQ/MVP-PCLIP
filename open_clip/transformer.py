...
#         super().__init__()
#         self.dim = dim
#         randvector = torch.empty((257,1,dim), dtype=torch.float32)
#         nn.init.normal_(randvector, std=0.1)
#         self.para = torch.nn.Parameter(randvector, requires_grad=True)
#         self.linear = nn.Linear(1024,1024)
#         # self.norm = nn.BatchNorm1d(257)
        

#     def forward(self, x):
#         # y = torch.cat([x,self.para],2)
#         y = x+ self.para
#         # y = x
#         y = self.linear(y)
...
#         super().__init__()
#         self.dim = dim
#         randvector = torch.empty((dim,dim,16,16), dtype=torch.float32)
#         nn.init.normal_(randvector, std=0.1)
#         self.para = torch.nn.Parameter(randvector, requires_grad=True)
#         self.cov = nn.Conv2d(1024+dim,1024,(3,3),1,1)
#         # self.norm = nn.LayerNorm(1024)
        

#     def forward(self, x):
#         feature_map = x[1:,:,:]
#         L, B, C = feature_map.shape
#         H = int(np.sqrt(L))
#         feature_map = feature_map.permute(1, 2, 0).reshape(B, C, H, H)
#         y = torch.cat([feature_map,self.para],1)
#         y = self.cov(y)
#         # y = self.norm(y)
#         y = y + feature_map
#         # x = self.norm(x)
#         y = y.reshape(B,C,L).permute(2,0,1)
#         y = torch.cat([x[0,:,:].unsqueeze(0),y],0)
#         return y
...
class prompt_block(nn.Module):
    def __init__(self,num):
        ...
        randvector = torch.empty((num,1,1024), dtype=torch.float32)
        nn.init.normal_(randvector, std=0.2)
        self.para = torch.nn.Parameter(randvector, requires_grad=True)


    def forward(self, x):
        # x = x[:257,:,:]
        y = torch.cat([x,self.para],0)
        return y
...
class Transformer(nn.Module):
    def __init__(
        ...
        self.viprompt = viprompt
        if not viprompt == None:
            self.prompt_blocks = nn.ModuleList([
                prompt_block(viprompt['tokens']) for idx in range(viprompt['nums'])
            ])
    ...
    def forward(self, x: torch.Tensor, out_layers: list = [3, 6, 9],
        ...
        for r in self.resblocks:
            ...
            if self.grad_checkpointing and not torch.jit.is_scripting():
                ...
                if idx == 12:
                    if not self.viprompt ==None and idx in self.viprompt['list']:
                        x = self.prompt_blocks[idx-1](x)
                        x, attn = r(x, attn_mask=attn_mask)
                        x = x[:257,:,:]
                        attn = attn[:,0:257,0:257]
                    else:
                        x, attn = r(x, attn_mask=attn_mask)
